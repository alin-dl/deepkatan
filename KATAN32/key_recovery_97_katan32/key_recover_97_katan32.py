...
import numpy as np
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"#GPU selected for computation
...
net50.load_weights('1627922944best84depth.h5');

json_file1 = open('1627922944model77.json','r');
json_model1 = json_file1.read();
net43 = model_from_json(json_model1);
net43.load_weights('1627922944best77depth.h5');

                 
m50 = np.load('data_wrong_key_mean_97r.npy');
s50 = np.load('data_wrong_key_std_97r.npy'); s50 = 1.0/s50;
m43 = np.load('data_wrong_key_mean_90r.npy');
s43 = np.load('data_wrong_key_std_90r.npy'); s43 = 1.0/s43;
...
def convert_to_binary(l):
  ...
  l = l.transpose() 
  #print(len(l[0]))
  X = np.zeros((1024, len(l[0])),dtype=np.uint8);
  #print(X)
  #print(len(X))
  for i in range(1024):
    index = i // 32;
...
def gen_key(nr):
  keys = np.frombuffer(urandom(10),dtype=np.uint16).reshape(5,-1);
  keys = np.copy(keys);
  myKATAN = ka.KATAN(keys, 32, nr);
  all_key=myKATAN.change_key(keys);
  # 30 bit subkeys consisting of all keys to be recovered
  ks1 = 0x0^(all_key[176]<<15)^(all_key[178]<<14)^(all_key[180]<<13)^(all_key[181]<<12)^(all_key[182]<<11)^(all_key[183]<<10)^(all_key[184]<<9)^(all_key[185]<<8)^(all_key[186]<<7)^(all_key[187]<<6)^(all_key[188]<<5)^(all_key[189]<<4)^(all_key[190]<<3)^(all_key[191]<<2)^(all_key[192]<<1)^(all_key[193]);  
  ks2 = 0x0^(all_key[162]<<15)^(all_key[164]<<14)^(all_key[166]<<13)^(all_key[167]<<12)^(all_key[168]<<11)^(all_key[169]<<10)^(all_key[170]<<9)^(all_key[171]<<8)^(all_key[172]<<7)^(all_key[173]<<6)^(all_key[174]<<5)^(all_key[175]<<4)^(all_key[176]<<3)^(all_key[177]<<2)^(all_key[178]<<1)^(all_key[179]);
  return(keys,ks1,ks2);

def gen_challenge(n, nr, diff=(0x61082200), keyschedule='real'):
...
def verifier_search(cts, best_guess, use_n = 32, net = net43):
  ...
  ck1 = np.repeat(ck1, n*32); keys1 = np.copy(ck1);#137*137
  ck2 = np.repeat(ck2, n*32); keys2 = np.copy(ck2);
  ck1 = np.repeat(ck1, use_n);
  ck2 = np.repeat(ck2, use_n);
  cta = np.tile(cts[0][0:32*use_n], n*n);
  ctb = np.tile(cts[1][0:32*use_n], n*n);
  cta1,ctb1 = ka.dec_rounds1(cta,ctb,ck1)# 7 rounds of decryption using the guessed key
  ctdata = ka.dec_rounds2(cta1,ctb1,ck2) # Decrypt another 13 rounds
...
'''
#Generate 0~16383, each number has 32
...
def bayesian_key_recovery(cts, net=net50, m = m50, s = s50, num_cand = 32, num_iter=5, seed = None):
  ...
  for i in range(num_iter):
    k = np.repeat(keys, n);
    ctdata = ka.dec_rounds(cta, ctb, k); # Decrypt 13 rounds 
    #print(len(ctdata))
    X = convert_to_binary(ctdata);
...
def test(n, nr=97, num_structures=100, it=100, cutoff1=5.0, cutoff2=50.0, keyschedule='real',net=net50, net_help=net43, m_main=m50, s_main=s50,  m_help=m43, s_help=s43, verify_breadth=None):
  print("Checking KATAN32 implementation."); 
  ...
  for i in range(n):
    ...
    #print(guess)
    num_used = min(num_structures, num_used); data = data + 2 * (32*32) * num_used;
    print(data)
